...
Nibiru supports the proposal types outlined below. These types are registered and processed through the governance module (eg. `ParamChangeProposal`), which then execute the respective module's proposal handler when a proposal passes. Governance handlers can perform arbitrary state changes.
